  ...
  try {
    ...
    const [salesRes, owedRes] = await Promise.all([
      supabaseClient.from('transactions').select('total_amount')
        .eq('business_id', TEST_BUSINESS_ID).is('deleted_at', null).gte('created_at', since.toISOString()),
      supabaseClient.from('ledger_entries').select('amount_owed, direction')
        .eq('business_id', TEST_BUSINESS_ID).eq('status', 'current').is('deleted_at', null)
    ]);
  ...
  try {
    const [salesRes, purchasesRes, payablesRes] = await Promise.all([
      supabaseClient.from('transactions').select('*, customers(name)')
        .eq('business_id', TEST_BUSINESS_ID).is('deleted_at', null).order('created_at', { ascending: false }).limit(20),
      supabaseClient.from('expenses').select('*')
        .eq('business_id', TEST_BUSINESS_ID).eq('category', 'Stock Purchase').is('deleted_at', null)
        .order('created_at', { ascending: false }).limit(20),
      supabaseClient.from('ledger_entries').select('*, suppliers(name)')
        .eq('business_id', TEST_BUSINESS_ID).eq('direction', 'payable').eq('status', 'current').is('deleted_at', null)
    ]);
